    ...
    private void Update()
    {
        ...
        if (_enableMovement)
        {
            ...
            if (Input.GetKey(_boostSpeed))
                currentSpeed = _boostedSpeed * Time.deltaTime;

            if (Input.GetKey(KeyCode.W))
                deltaPosition += transform.forward * Time.deltaTime;

            if (Input.GetKey(KeyCode.S))
                deltaPosition -= transform.forward * Time.deltaTime;

            if (Input.GetKey(KeyCode.A))
                deltaPosition -= transform.right * Time.deltaTime;

            if (Input.GetKey(KeyCode.D))
                deltaPosition += transform.right * Time.deltaTime;

            if (Input.GetKey(_moveUp))
                deltaPosition += transform.up * Time.deltaTime;
        ...
        if (_enableRotation)
        {
            ...
            transform.rotation *= Quaternion.AngleAxis(
                -Input.GetAxis("Mouse Y") * _mouseSense * Time.deltaTime,
                Vector3.right
            );
            ...
            transform.rotation = Quaternion.Euler(
                ...
                transform.eulerAngles.y + Input.GetAxis("Mouse X") * _mouseSense * Time.deltaTime,
