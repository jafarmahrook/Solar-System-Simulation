...
public class FlyBehaviour : GenericBehaviour
{
	...
	void Start()
	{
        ...
        // Automatically enter fly mode at the start of the game
        fly = true;
    ...
    void Update()
    {
        // Toggle fly by input, only if there is no overriding state or temporary transitions, and fly mode is not already active.
        if (Input.GetButtonDown(flyButton) && !behaviourManager.IsOverriding()
            && !behaviourManager.GetTempLockStatus(behaviourManager.GetDefaultBehaviour) && !fly)
        {
            fly = !fly;
